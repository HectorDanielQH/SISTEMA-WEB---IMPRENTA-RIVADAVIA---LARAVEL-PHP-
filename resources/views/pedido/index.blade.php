...
<nav>
  <div class="nav nav-tabs" id="nav-tab" role="tablist">
    <a class="nav-item nav-link active" id="nav-home-tab" data-toggle="tab" href="#nav-home" role="tab" aria-controls="nav-home" aria-selected="true">Pedidos Por Descripcion</a>
    <a class="nav-link" id="nav-profile-tab" data-toggle="tab" href="#nav-profile" role="tab" aria-controls="nav-profile" aria-selected="false">Pedidos por Formulario</a>
  </div>
</nav>
<div class="tab-content" id="nav-tabContent">    
        ...
        <table class="table table-dark" style="width:100%">
            ...
            <tbody class='fila'>
                ...
                @forelse($pedido as $pedidos)
                    <tr>
                        <td class="border-solid border-2 border-gray-500 text-xs px-4 py-2">Descripcion del pedido: <h3>{{ $pedidos->descripcion }}</h3> en fecha: <h5>{{$pedidos->created_at->year}}-{{$pedidos->created_at->month}}-{{$pedidos->created_at->day}}</h5></td>
                        ...
                        <td class="border-solid border-4 border-gray-500 text-xs px-4 py-2">
                            <div class="inline-flex">
                                <a href="{{ route('pedido.show', $pedidos->numero_pedido) }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-l">
                                    <h5>{{__("Visualizar Formulario de Pedido")}}</h5>
                                </a>
                            </div>
                        </td>
                    </tr>
    ...
    <div class="input-group justify-content-center align-items-center">
            <h4>{{ __("Busca por numero de pedido") }}</h4>
        ...
        <table class="table table-dark" style="width:100%">
            ...
            <tbody class='filados'>
                @forelse($pedido2 as $pedidos)
                    <tr>
                        <td class="border-solid border-2 border-gray-500 text-xs px-4 py-2">Formulario del pedido Numero: <h3>{{ $pedidos->numero_pedido }}</h3> Creado por: {{$pedidos->clientes->nombres}} {{$pedidos->clientes->apellidos}}</td>
...
@section('js')
                ...
                for(let i=0;i<arreglo.length;i++){
                    ...
                    <tr>
                        ...
                        <td class="border-solid border-2 border-gray-500 text-xs px-4 py-2">
                            <div class="inline-flex">
                            <a href="pedido/${arreglo[i]['numero_pedido']}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-l">
    ...
    <script defer>
        ...
        $("#tiempoactualdos").change(function(){
            ...
            tiempodos=$(this).val();
            $.ajax({
                url:`pedidobusform/${textodos}/${tiempodos}`,
                method:'GET',
                data:{
                    text:textodos,
                ...
                for(let i=0;i<arreglo.length;i++){
                    cadena+=`
                    <tr>
                        <td class="border-solid border-2 border-gray-500 text-xs px-4 py-2">Formulario del pedido Numero: <h3>${arreglo[i]['numero_pedido']}</h3>Crceado por: {{auth()->user()->clientes}} {{auth()->user()->clientes}}</td>
                        <td class="border-solid border-2 border-gray-500 text-xs px-4 py-2">
                            <div class="inline-flex">
                            <a href="pedido/${arreglo[i]['numero_pedido']}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-l">
                                {{__("Visualizar Formulario de Pedido")}}
